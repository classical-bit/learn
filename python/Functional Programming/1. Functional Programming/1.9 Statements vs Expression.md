...
Studying functional programming is really about returning to the most basic aspects of programming and looking at them in a new way. Statements and expression are a great example of that.
...
## Expressions
*Expressions* are a subset of statements that *produce* values. Evaluating an expression results in a value can be used in whatever way is needed. It can be assigned to a variable, returned from a function, etc.
...
```
One thing that may suprise you is that, in most languages (including Python), every function call is an expression. When you call a function, it returns a value -- whether or not you realize it or do anything with that value.
...
Because expressions always produce values, they're reusable and declarative. You can compose expressions and nest them within each other -- but you can't always do that with other kind of statements.
...
Again, it's simple to combine expression:
...
Expressions tend to be concise and logically pure. Some languages that are designed for programming, like Haskell, treat everything as an expression. In those languages, even control flow constructs like `if` and `case` are expressions that return values.
